...
export interface TestScoreRecord {
  id: string; // Firestore document ID of the source record (e.g., PhieuLienLacRecord)
  studentId: string;
  classId: string; 
...
export interface QuestionOption {
  id: string; // A, B, C, D or unique ID
  text: string;
}

export interface Question {
  id: string; // Unique ID for the question (e.g., generated by nanoid)
  text: string;
  options: QuestionOption[]; // Typically 4 options
